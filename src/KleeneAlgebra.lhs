...
contained the star operation of a non-zero and non-one element.
Since we don't require any of the algebraic rules in the implementation, we omit this
definition.
Still, need to implement some algebraic rules concerning the constants to (greatly!)
improve the performance in case of the Kleene closure computation of a matrix.
In particular, we deal with recognising `star one = one` and `star zero = one` as well
as the neutrality of `one` with respect to `(.*.)`, the neutrality of `zero` with respect
to `(.+.)` and the annihilation property of `zero` with respect to `(.*.)`. 
...
"Semirings for Social Network Analysis", Journal of Mathematical Sociology, pp. 53 -- 68) can be
used to check the types of paths in a graph.
A signed graph /(G = (V, E), sign)/ is a graph equipped with a function /sign : E -> {P, N}/ where
/P/ stands for /positive/ and /N/ stands for /negative/.
A walk in a graph is called positive (resp. negative) iff it contains an even (resp. odd) number of
negative arcs.
A semiwalk is a walk in the symmetric closure of the graph.
A graph is called /balanced/ iff there is a partition of /V/ in two sets /A, B/ such that
for all edges /e/ we have
    * / sign e == P / ==> /e in A x A/ or /e in B x B/
    * / sign e == N / ==> /e in A x B/ or /e in B x A/
That is: every positive edge connects vertices of the same partition set and every negative edge
connects vertices of different partition sets (cf. the above mentioned source for more detail).
Balanced graphs can be characterised as follows: a graph is balanced iff every closed semiwalk is
positive.
...
We can define a semiring instance for `Balance`.
The basic idea is to think about the composition of paths as multiplication and the alternative of
paths as addition.
...
The star closures of `NoPath, Negative` and `Mixed` can be easily computed using the fixpoint
equation `star x = one .+. x .*. star x`.
The equation `star Positive = Positive` is true because of the geometric series representation of
the star closure.
